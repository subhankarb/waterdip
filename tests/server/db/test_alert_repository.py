...
class TestAlertRepository:
    ...
    def test_should_count_alerts_by_monitor_type_per_model(cls, mock_mongo_backend : MongodbBackend):
        agg_pipeline = [
            {
                '$match': {
                    'model_id': {
                        '$in': [str(model_id) for model_id in cls.model_ids]
                    }
                }
            }, {
                '$group': {
        ...
        alerts_count = list(cls.alert_repository.agg_alerts(agg_pipeline=agg_pipeline))
        for alert in alerts_count:
            if(alert["_id"] == str(cls.model_ids[0])):
            ...
            if(alert["_id"] == str(cls.model_ids[1])):
                if alert["alerts"][0]["monitor_type"] == MonitorType.DRIFT:
                    assert alert["alerts"][0]["count"] == 2
                if alert["alerts"][1]["monitor_type"] == MonitorType.MODEL_PERFORMANCE:
                    assert alert["alerts"][1]["count"] == 1
